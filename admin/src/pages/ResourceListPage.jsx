...
export default function ResourceListPage() {
  ...
  const load = useCallback(() => {
    ...
    setLoading(true);
    config.endpoint
      .list({ page, search, ordering, ...filterValues, ...parentFilter })
      .then(({ data }) => setData(data))
      .finally(() => setLoading(false));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [config, page, search, ordering, JSON.stringify(filterValues), JSON.stringify(parentFilter)]);
    ...
    ...config.columns,
    {
      ...
      render: (row) => (
        <div className="res-row-actions">
          <button className="res-row-btn" onClick={(e) => { e.stopPropagation(); openEdit(row); }} title="Edit">
      ...
      <div className="res-toolbar">
        {config.search && (
          <div className="res-search">
            <Input placeholder={tr.search} value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} />
          </div>
        )}
        {config.filters?.map((f) => (
          <div className="res-filters" key={f.key}>
            <Segmented
